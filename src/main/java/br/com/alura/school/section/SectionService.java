...
public class SectionService {
    ...
    public URI addNewSection(String code, NewSectionRequest newSectionRequest) {
        ...
        if (author.getRole() != UserRole.INSTRUCTOR)
            throw new ResponseStatusException(BAD_REQUEST, format("User %s is not a instructor", author.getUsername()));
    ...
    public void addNewVideo(String code, String sectionCode, NewVideoRequest newVideoRequest) {
        ...
        if (section.getVideos().contains(video))
            throw new ResponseStatusException(BAD_REQUEST, format("Video %s already on class", video));
        section.addVideo(video);
        sectionRepository.save(section);
    }

    public List<Section> findAllByCourse(Course course) {
        return sectionRepository.findAllByCourse(course).orElseThrow(() -> new ResponseStatusException(NOT_FOUND, format("Course with code %s has no classes yet", course.getCode())));
    }
